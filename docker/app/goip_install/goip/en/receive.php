...
$select.="<input type=\"submit\" value=\"Search\">\n";
        ...
        $fenye=showpage("receive.php?action=$action&column=$column&type=$type&s_key=$s_key&",$page,$count,$perpage,true,true,"row(s)");
	...
	while($row=$db->fetch_array($query)){
		...
		if($row[status]==0)
			$row[status]='<font color="#FF0000">not read</font>';
		else 
			$row[status]='read';
		$rsdb[]=$row;
