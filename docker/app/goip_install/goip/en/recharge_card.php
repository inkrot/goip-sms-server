...
if(isset($_GET['action'])) {
	...
	if($action=="del")
	{
					...
					if($ErrMsg!="")
						...
						WriteSuccessMsg("<br><li>Delete success</li>","recharge_card.php");
	...
	elseif($action=="saveadd")
	{
		...
		$ErrMsg="";
		if(empty($card))
			$ErrMsg ='<br><li>Plesase input card number.</li>';
		if($ErrMsg!="")
			WriteErrMsg($ErrMsg);
		else{

			$db->query("INSERT INTO recharge_card (card,prov_id) VALUES ('$card','$prov_id')");
			WriteSuccessMsg("<br><li>Add successfully</li>","recharge_card.php");				
		}
	}
	elseif($action=="savemodify")
	{
		...
		if($ErrMsg!="")
			...
			WriteSuccessMsg("<br><li>Modify Add successfully</li>","recharge_card.php");
		}
	}
		...
		if ( $_POST["action"]=="mingsenupload")
		{
			...
			if($attach_saved == 1){
				...
				if($ext=='.csv')
				{
					...
					fclose($fp); 
					$Msg= "<br><li>Import cards is done, total $row cards</li>".$Msg;
				...
				else if($ext=='.xls') 
				{
					...
					//--$row;  
					$Msg= "<br><li>Import cards is done, total $srow cards</li>".$Msg; 
					WriteSuccessMsg($Msg,"recharge_card.php");
				}
			}  
		}
...
if($action=="main")
{
	...
	$fenye=showpage("?order_key=$_REQUEST[order_key]&order=$_REQUEST[order]&",$page,$count,$perpage,true,true,"row");
